...
export const replacements = sqliteTable(
	...
	(table) => [
		index('replacements_user_id_idx').on(table.userId),
		index('replacements_deadname_idx').on(table.deadname),
		index('replacements_email_idx').on(table.email),
		index('replacements_hash_idx').on(table.hash)
	]
);
